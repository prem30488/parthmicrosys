...
const AddProduct = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (res.data.success) {
                toast.success("Product created! Deployment in progress...");
                router.push("/pdms/products");
            }
        } catch (err) {
        ...
        <div className="max-w-3xl mx-auto animate-fade-in">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="flex items-center justify-end gap-3">
                    <button type="button" onClick={() => router.push("/pdms/products")} className="px-5 py-2.5 text-dark-300 hover:text-white hover:bg-dark-800 rounded-xl text-sm font-medium transition-colors">Cancel</button>
                    <button type="submit" disabled={loading} id="create-product-btn"
                        ...
                        {loading ? (
                            ...
                            <><HiOutlineCloudUpload className="w-4 h-4" />Create &amp; Deploy</>
                        )}
                    </button>
                </div>
            </form>
